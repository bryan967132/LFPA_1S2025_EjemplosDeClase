...
public class App {
    ...
    public static void cargarArchivo(String ruta) {
        try (BufferedReader br = new BufferedReader(new FileReader(ruta))) {
            ...
            ArrayList<Pelea> torneo = new ArrayList<>();
            Jugador jug1, jug2 = null;
            for(int i = 0; i < jugadores.size(); i += 2) {
                System.out.println(i);
                jug1 = jugadores.get(i);
                if(i < jugadores.size() - 1) {
                    jug2 = jugadores.get(i + 1);
                }

                torneo.add(new Pelea(jug1, jug2));
                jug1 = null;
                jug2 = null;
            }

            for(Pelea pelea : torneo) {
...
class Pelea {
    ...
    public void iniciar() {
        if(j2 == null) {
            System.out.println("Gana Jugador 1 (Default): " + j1.nombre);
            return;
        }
        ...
        if(dano1 > 0 || dano2 > 0) {
            ...
            // elegir un jugador aleatorio para que gane
        }
    }
}
